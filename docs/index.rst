.. meta::
  :description: hipMM documentation and API reference library
  :keywords: hipMM, RMM, data science, RAPIDS, AMD Data Science, GPU, GPU API, memory-management, memory-allocation, memory-pools

.. _hipMM-index:

********************************************************************
HIP Memory Manager (hipMM) documentation
********************************************************************

.. note::
   hipMM is in an early access state. Running production workloads is not recommended.

The HIP memory manager (hipMM) library supports the core AMD Data Science libraries by providing a common interface for custom
host and device memory allocation. hipMM is derived from NVIDIA RAPIDS™ open source project RMM.

The hipMM code is open and hosted at `https://github.com/AMD-Ecosystem/hipMM <https://github.com/AMD-Ecosystem/hipMM>`_.

The hipMM documentation is structured as follows:

.. grid:: 2
  :gutter: 3

  .. grid-item-card:: Installation

    * `Install hipMM <./install/INSTALL.html>`_

To contribute to the documentation refer to `Contributing to AMD Data Science  <https://rocm.docs.amd.com/projects/rocm-ds/en/docs-25.05/contribute/contributing.html>`_.

You can find licensing information on the `Licensing <https://rocm.docs.amd.com/projects/hipMM/en/docs-25.05/license.html>`_ page.
